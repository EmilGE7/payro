...
def generate_payslip_pdf(payroll_record):
    ...
    pdf.ln(10)
    pdf.set_font('DejaVu', 'B', 12); pdf.cell(0, 10, 'Salary Breakdown', ln=True, fill=True)
    pdf.set_font('DejaVu', 'B', 10); pdf.cell(100, 10, 'Description', border=1); pdf.cell(90, 10, 'Amount (INR)', border=1, ln=True, align='R')
    pdf.set_font('DejaVu', '', 10)
    ...
    pdf.set_text_color(0); pdf.set_font('DejaVu', 'B', 12); pdf.cell(100, 12, 'NET PAYABLE', border=1); pdf.cell(90, 12, f'₹{payroll_record.net_amount:,.2f}', border=1, ln=True, align='R')
    pdf.ln(20); pdf.set_font('DejaVu', '', 10); pdf.multi_cell(0, 5, 'Computer-generated document. No signature required.')
    return pdf.output()
...
def ai_monthly_report_pdf():
    ...
    pdf.set_font('DejaVu', '', 11); pdf.set_text_color(100); pdf.cell(0, 7, month, ln=True); pdf.ln(10)
    pdf.set_font('DejaVu', '', 12); pdf.set_text_color(30, 30, 30); pdf.multi_cell(0, 8, narrative); pdf.ln(15)
    pdf.set_font('DejaVu', '', 9); pdf.set_text_color(150); pdf.cell(0, 5, f'Generated by Payro AI on {datetime.now().strftime("%Y-%m-%d %H:%M")}', ln=True)
